...
DB_CONFIG = {
    ...
    'user': 'postgres',
    'password': 'Telesforo123',  # Cambiar por tu contraseña
    'port': 5432
}
...
class DataGenerator:
    ...
    def generate_drivers(self, count=400):
        ...
        query = """
            insert into persons.drivers (employee_code, first_name, last_name, 
    ...
    def generate_routes(self, count=50):
        ...
        query = """
            insert into resources.routes (route_code, origin_city, destination_city, 
    ...
    def generate_trips(self, count=100000):
        ...
        # Obtener IDs válidos
        self.cursor.execute("select vehicle_id, capacity_kg from resources.vehicles where status = 'active'")
        vehicles = self.cursor.fetchall()
        
        self.cursor.execute("select driver_id from persons.drivers where status = 'active'")
        drivers = [d[0] for d in self.cursor.fetchall()]
        
        self.cursor.execute("select route_id, distance_km, estimated_duration_hours from resources.routes")
        ...
        # Insertar en batches
        query = """
            insert into resources.trips (vehicle_id, driver_id, route_id, departure_datetime,
    ...
    def generate_deliveries(self, count=400000):
        ...
        self.cursor.execute("""
            ...
            inner join resources.routes r on t.route_id = r.route_id
        """)
    ...
    def generate_maintenance(self, count=5000):
        ...
        # Insertar en batch
        query = """
            insert into resources.maintenance (vehicle_id, maintenance_date, maintenance_type,
    ...
    def validate_data_quality(self):
        ...
        validations = {
            "Integridad referencial - Trips sin vehículo válido": """
                select count(*) from resources.trips t 
                left join resources.vehicles v on t.vehicle_id = v.vehicle_id 
                where v.vehicle_id is null
            """,
            "Integridad referencial - Deliveries sin trip válido": """
                select count(*) from resources.deliveries d 
                left join resources.trips t on d.trip_id = t.trip_id 
            ...
            "Consistencia de peso - Trips excediendo capacidad": """
                select count(*) from resources.trips t 
                inner join resources.vehicles v on t.vehicle_id = v.vehicle_id 
                where t.total_weight_kg > v.capacity_kg
            """,
            "Entregas sin tracking number": """
                select count(*) 
                from resources.deliveries d 
                where d.tracking_number is null or d.tracking_number = ''
            """
        }
    ...
    def generate_summary_report(self):
        ...
        self.cursor.execute("""
            ...
            from (
                ...
                from resources.deliveries d 
	            group by trip_id) as delivery_stats
        """)
